...
class Director {
    ...
    val media : CoreMedia
        get() = mediaBuilder!!.getMediaHandle()!!   // we are getting the CoreMedia handle which points to the various elements of the phone
...
////  Very promising but not right...

interface USER< out R >{
    fun visit( mobile : CoreMedia )
}

// create an extended class to find the type of the media if its s10 then
// get the data.

class get_cost : USER< Double > {
    override fun visit(mobile: CoreMedia) {
        var price =mobile.codecCost+
                mobile.streamerVLCCost+
                mobile.surfaceFlingerCost+
                mobile. pluginCost+
                mobile.skinCost

        println("total sum of SAMSUNG S10:- $price")
